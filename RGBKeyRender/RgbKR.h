...
extern "C"
{
    //Gets the width of the keyboard divided by the height.
    //This should be the aspect ratio of your image.
    RKR_API float rkrGetAspectRatio();
    ...
    RKR_API int rkrRenderToKeyboard(uint32_t imgWidth, uint32_t imgHeight, const RGBKey* texture);
    //The "rgbaTexture" should be a set of rkrGetWidth() * rkrGetHeight() pixels,
    //    left to right then top to bottom,
    //    where each pixel contains three one-byte components: Red, Green, and Blue.
    //Returns 0 if it succeeds, and a Razer Chroma SDK error code if it fails.
    RKR_API int rkrRenderToKeyboardSimple(const RGBKey* texture);


    //Cleans up the library behind this interface.
    //Returns 0 if it succeeds, and a Razer Chroma SDK error code if it fails.
    RKR_API int rkrCleanUp();
}